...
class ResNet(nn.Module):
    def __init__(self, variant: str = '50', pretrained: str = None, num_classes: int = 50) -> None:
        super().__init__()
        self.inplanes = 64

        assert variant in resnet_settings.keys(), f"ResNet model name should be in {list(resnet_settings.keys())}"
        block, layers = resnet_settings[variant]
